...
type Coupon = {
  ...
  is_dropshipper_exclusive?: boolean | null;
};
...
function CouponsAdmin() {
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      is_dropshipper_exclusive: editing.is_dropshipper_exclusive ?? false,
    };
          ...
          <table className="w-full text-sm">
            <thead className="bg-purple-50/50 text-[10px] uppercase tracking-widest text-purple-800/70">
              <tr>
                ...
                <th className="px-3 py-3 text-left font-bold">Exclusive</th>
            ...
            <tbody>
              {list.map((c) => (
                <tr key={c.id} className="border-t border-slate-100 hover:bg-purple-50/30">
                  ...
                  <td className="px-3 py-2.5">
                    {c.is_dropshipper_exclusive && <Badge tone="rose">DS Only</Badge>}
                  </td>
        ...
        <Modal onClose={() => setEditing(null)}>
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <TextInput type="number" value={editing.usage_limit ?? ""} onChange={(e) => setEditing({ ...editing, usage_limit: e.target.value ? Number(e.target.value) : null })} placeholder="Redemption limit" />
            </div>
            ...
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input type="checkbox" checked={editing.is_dropshipper_exclusive ?? false} onChange={(e) => setEditing({ ...editing, is_dropshipper_exclusive: e.target.checked })} />
              Dropshipper Exclusive
            </label>
            <ProductPicker
