...
export const Route = createFileRoute("/p/$slug")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} — Bazar BD` },
      { name: "description", content: `Buy ${params.slug.split('-').join(' ')} at the best price on Bazar BD. Quality products with fast delivery.` },
      { property: "og:title", content: params.slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') },
      { property: "og:description", content: "Shop the latest collections on Bazar BD." },
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <h1 className="mt-3 text-lg font-extrabold">প্রোডাক্টটি পাওয়া যায়নি / Product unavailable</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          প্রোডাক্টটি নিষ্ক্রিয় করা হয়েছে বা লিংকটি ভুল। একই ধরনের প্রোডাক্ট খুঁজে দেখুন।
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <Link to="/search" search={{ q: "" }} className="rounded-full bg-primary px-5 py-2 text-sm font-bold text-primary-foreground hover:opacity-90">সার্চ করুন</Link>
          <Link to="/categories" className="rounded-full border px-5 py-2 text-sm font-bold hover:bg-muted">ক্যাটাগরি</Link>
          <Link to="/" className="rounded-full border px-5 py-2 text-sm font-bold hover:bg-muted">হোম</Link>
        </div>
      </div>
    </SiteLayout>
  ),
...
function PublicProductPage() {
  ...
  const requireAuthForCart = () => {
    ...
    navigate({ to: "/auth", search: { redirect: window.location.pathname, mode: "login" } });
    return false;
  };
    ...
    return (
      <SiteLayout>
        <div className="mx-auto max-w-md px-4 py-16 text-center">
          ...
          <h1 className="mt-3 text-lg font-extrabold">প্রোডাক্টটি পাওয়া যায়নি / Product unavailable</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {err || "প্রোডাক্টটি নিষ্ক্রিয় করা হয়েছে বা লিংকটি ভুল।"}
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            <Link to="/search" search={{ q: "" }} className="rounded-full bg-primary px-5 py-2 text-sm font-bold text-primary-foreground hover:opacity-90">সার্চ করুন</Link>
            <Link to="/categories" className="rounded-full border px-5 py-2 text-sm font-bold hover:bg-muted">ক্যাটাগরি</Link>
  ...
  const handleManualMediaChange = (index: number) => {
    setActive(index);
  };
  ...
  const cartProduct: any = {
    ...
    price, mrp: mrp || price, brand: p.brand || "", category: String(p.category_slug || "").split(",")[0].trim(),
  ...
  return (
    <SiteLayout footer={<Footer />}>
      <div className="mx-auto w-full max-w-6xl overflow-x-hidden px-3 py-3 md:px-4 md:py-4">
        <nav className="mb-3 hidden flex-wrap items-center gap-1 text-xs text-muted-foreground md:flex">
          <Link to="/" className="hover:text-primary">Home</Link>
          <ChevronRight className="h-3 w-3" />
          {p.category_slug && <><span className="capitalize">{String(p.category_slug).split(",")[0].trim()}</span><ChevronRight className="h-3 w-3" /></>}
          <span className="truncate font-medium text-foreground">{p.name}</span>
        </nav>

        <div className="flex flex-col gap-3 md:grid md:gap-5 md:grid-cols-[minmax(0,440px)_minmax(0,1fr)]">
          {/* Gallery */}
          <div className="min-w-0">
            <SwipeGallery
              ...
              showArrows={activeMedia?.type !== "video"}
              className="relative aspect-square w-full touch-pan-y select-none overflow-hidden rounded-2xl border bg-gradient-to-br from-sky-50 to-pink-50"
            >
              ...
              <button
                onClick={() => toggle(p.id)}
                className={`absolute right-3 top-3 z-10 grid size-10 place-items-center rounded-full shadow ${wished ? "bg-primary text-white" : "bg-white/90 text-foreground"}`}
              >
                <Heart className={`size-5 ${wished ? "fill-current" : ""}`} />
              </button>
              {discount > 0 && (
                <span className="absolute left-3 top-3 z-10 rounded-full bg-red-500 px-2 py-0.5 text-xs font-bold text-white shadow">-{discount}%</span>
              )}
            </SwipeGallery>
            {media.length > 1 && (
              <ThumbnailStrip className="mt-2 shrink-0 md:mt-3">
                {media.map((m, i) => (
                  <button
                    key={i}
                    onClick={() => handleManualMediaChange(i)}
                    className={`relative size-16 shrink-0 snap-start overflow-hidden rounded-lg border-2 md:size-20 ${i === active ? "border-primary" : "border-transparent opacity-70 hover:opacity-100"}`}
                  >
                    {m.type === "image" ? (
                      ...
                      <>
                        ...
                        <span className="absolute inset-0 grid place-items-center bg-black/30">
                          <span className="grid size-8 place-items-center rounded-full bg-white/90 text-primary shadow">
          ...
          {/* Info */}
          <div className="flex min-w-0 flex-col gap-2 md:block md:space-y-3">
            ...
            <h1 className="text-base font-bold leading-tight md:text-xl md:leading-snug lg:text-2xl">{p.name}</h1>
            ...
            <div className="py-0.5 md:py-1">
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-extrabold text-[#A52A2A] dark:text-[#E9967A] leading-none whitespace-nowrap md:text-[34px]">{formatBDT(price)}</span>
                {mrp > price && <span className="text-xs text-muted-foreground line-through md:text-sm">{formatBDT(mrp)}</span>}
              </div>
              {discount > 0 && (
            ...
            <div className={`inline-flex w-fit items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-bold md:px-2.5 md:py-1 md:text-xs ${stockInfo.className} ${stockInfo.outOfStock ? "border-red-200 bg-red-50" : stockInfo.permanent ? "border-emerald-200 bg-emerald-50" : liveStock <= 5 ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
            ...
            <div className="flex gap-2 pt-2">
              {([
                ...
                <button
                  ...
                  className={`flex-1 rounded-full bg-gradient-to-r ${b.cls} py-2 text-sm font-bold text-white shadow transition hover:brightness-110 disabled:cursor-not-allowed disabled:from-slate-400 disabled:via-slate-400 disabled:to-slate-500 disabled:opacity-70 md:py-3 md:text-base lg:text-lg`}
                >
