...
import { hasSlug, slugList } from "@/lib/slugs";
...
export const Route = createFileRoute("/category/$slug")({
  ...
  loader: async ({ params }) => {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("id,slug,name,price,original_price,rating,sold_count,category_slug,category_name,subcategory_slug,subcategory_name,option_slug,brand,image,gallery,description")
      .eq("is_active", true)
      .ilike("category_slug", `%${params.slug}%`)
      .limit(2000);
    if (error) {
    ...
    // `ilike %slug%` is only a coarse prefilter (a product's category_slug can
    // be a comma-separated multi-select list); match exactly here.
    const rows = (data || []).filter((r: any) => hasSlug(r.category_slug, params.slug));
    const derived = rows.length ? deriveLiveCategories(rows).find((c) => c.slug === params.slug) : undefined;
    if (derived) return { cat: derived, liveProducts: rows.map(toLiveProduct), fetchError: null };

    // No products yet — fall back to the curated categories table so the page
    // still renders with its real name + subcategories (empty state).
    const { data: catRows } = await supabase
      .from("categories")
      .select("id,name,slug,icon,parent_id,sort_order")
      .order("sort_order", { ascending: true });
    const all = catRows || [];
    const self = all.find((c: any) => c.slug === params.slug);
    if (!self) throw notFound();
    const kids = all.filter((c: any) => c.parent_id === self.id);
    const cat = {
      slug: self.slug,
      name: { bn: self.name, en: self.name },
      icon: self.icon && !String(self.icon).startsWith("http") ? self.icon : "🛍️",
      image: self.icon && String(self.icon).startsWith("http") ? self.icon : "",
      color: "from-primary/60 to-primary",
      subcategories: kids.map((s: any) => ({
        slug: s.slug,
        name: { bn: s.name, en: s.name },
        keyword: s.slug,
        children: all
          .filter((g: any) => g.parent_id === s.id)
          .map((g: any) => ({ slug: g.slug, name: { bn: g.name, en: g.name }, keyword: g.slug })),
      })),
    } as any;
    return { cat, liveProducts: [] as any[], fetchError: null };
  },
  head: ({ loaderData, params }) => ({
    meta: [
      { title: `${loaderData?.cat?.name?.en ?? params.slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} — Bazar BD` },
      { name: "description", content: `Explore the best collection of ${loaderData?.cat?.name?.en ?? params.slug.split('-').join(' ')} on Bazar BD. Quality products, fast delivery, and secure payments.` },
      { property: "og:title", content: loaderData?.cat?.name?.en ?? params.slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') },
      { property: "og:description", content: "Discover amazing deals on Bazar BD." },
      { name: "twitter:card", content: "summary_large_image" },
      { property: "og:type", content: "website" },
      ...(loaderData?.cat ? [] : [{ name: "robots", content: "noindex" }]),
    ],
  }),
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <h1 className="mt-3 text-lg font-extrabold">ক্যাটাগরিটি পাওয়া যায়নি / Category not found</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          এই ক্যাটাগরিটি সরানো হয়েছে বা লিংকটি ভুল। সব ক্যাটাগরি দেখুন অথবা সার্চ করুন।
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <Link to="/categories" className="rounded-full bg-primary px-5 py-2 text-sm font-bold text-primary-foreground hover:opacity-90">সব ক্যাটাগরি</Link>
          <Link to="/search" search={{ q: "" }} className="rounded-full border px-5 py-2 text-sm font-bold hover:bg-muted">সার্চ</Link>
...
function CategoryPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLiveExtra((data || []).filter((r: any) => hasSlug(r.category_slug, cat.slug)).map(toLiveProduct));
      setFetching(false);
    })();
    return () => { alive = false; };
  ...
  const list = useMemo(() => {
    ...
    const matchesSub = (p: any) => {
      const subs: string[] = p.subcategories?.length ? p.subcategories : slugList(p.subcategory);
      const opts: string[] = p.options ?? [];
      if (subs.includes(sub!) || opts.includes(sub!)) return true;
      // A level-3 option is often only stored via its level-2 parent slug.
      return !!subInfo.parent && subs.includes(subInfo.parent);
    };
    let l = sub ? all.filter(matchesSub) : all.slice();
  ...
  return (
    <SiteLayout>
      ...
      <div className="sticky top-[52px] z-20 hidden items-center gap-2 border-b border-border bg-card px-3 py-2">
      ...
      <div className="mx-auto max-w-none py-3 md:py-4">
        ...
        {/* Subcategory chips */}
        <div className="-mx-3 mb-2 hidden gap-2 overflow-x-auto border-b border-border bg-card px-3 py-2 no-scrollbar md:mx-0 md:flex md:rounded-md md:border">
          <button
            onClick={() => setSub(null)}
        ...
        {subInfo.children.length > 0 && (
          <div className="-mx-3 mb-2 hidden gap-2 overflow-x-auto border-b border-border bg-muted/30 px-3 py-2 no-scrollbar md:mx-0 md:flex md:rounded-md md:border">
        ...
        <div className="sticky top-[52px] z-10 -mx-3 mb-3 grid grid-cols-2 border-b border-border bg-card md:static md:mx-0 md:flex md:rounded-md md:border md:p-1">
          <button
