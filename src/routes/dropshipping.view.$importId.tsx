...
function ViewPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-4 md:px-4">
      <nav className="mb-3 hidden items-center gap-1 text-xs text-muted-foreground md:flex">
      ...
      <div className="grid gap-5 md:grid-cols-[minmax(0,440px)_minmax(0,1fr)]">
        {/* Gallery */}
        <div>
          <SwipeGallery count={images.length} index={active} onChange={handleManualMediaChange} className="relative aspect-square touch-pan-y select-none overflow-hidden rounded-2xl border bg-gradient-to-br from-sky-50 to-pink-50">
            ...
            <button onClick={share} className="absolute right-3 top-3 z-10 grid size-10 place-items-center rounded-full bg-white/90 shadow hover:bg-white"><Share2 className="h-4 w-4" /></button>
            <button className="absolute right-3 top-16 z-10 grid size-10 place-items-center rounded-full bg-white/90 shadow hover:bg-white"><Heart className="h-5 w-5" /></button>
            {discount > 0 && (
          ...
          {images.length > 1 && (
            <ThumbnailStrip className="mt-3">
              {images.map((src, i) => (
                <button key={i} onClick={() => handleManualMediaChange(i)} className={`relative size-16 shrink-0 snap-start overflow-hidden rounded-lg border-2 md:size-20 ${i === active ? "border-primary" : "border-transparent opacity-70 hover:opacity-100"}`}>
