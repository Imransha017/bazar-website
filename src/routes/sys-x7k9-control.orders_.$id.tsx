...
import { OrderWhatsAppNotify } from "@/components/admin/OrderWhatsAppNotify";
import { OrderRequestsCard } from "@/components/admin/OrderRequestsCard";
import { buildWhatsAppLink, renderTemplate, DEFAULT_TEMPLATES } from "@/lib/whatsapp";
...
function OrderDetails() {
  ...
  async function updateStatus(status: string) {
    ...
    // Also insert into order_activities for the timeline
    ...
    // Offer an instant WhatsApp update to the customer
    const { data: tplRow } = await supabase
      .from("whatsapp_templates")
      .select("message,is_active")
      .eq("status", status)
      .maybeSingle();
    const tpl = (tplRow as any)?.is_active === false ? null : ((tplRow as any)?.message ?? DEFAULT_TEMPLATES[status]);
    const link = tpl ? buildWhatsAppLink({ ...order, status } as any, renderTemplate(tpl, { ...order, status } as any, note ?? undefined)) : null;
    if (link) {
      toast.success("Status updated", {
        description: "গ্রাহককে WhatsApp-এ জানাতে চাপুন",
        action: { label: "WhatsApp", onClick: () => window.open(link, "_blank", "noopener") },
        duration: 10000,
      });
    } else {
      toast.success("Status updated");
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            {(order as any).source === "ai_assistant" && (
              <span
                title="Bazar AI Assistant কাস্টমারের হয়ে চ্যাট থেকে এই অর্ডারটি সাবমিট করেছে"
                className="inline-flex items-center gap-1 rounded-full bg-violet-100 px-2.5 py-1 text-[11px] font-bold text-violet-700"
              >
                🤖 AI Assistant অর্ডার
              </span>
            )}
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="space-y-5">
          <OrderWhatsAppNotify order={order as any} />
          <OrderRequestsCard orderNumber={order.order_number} />
          {partner && (
            <Surface>
          ...
          <Surface>
            <SectionTitle icon={Handshake}>Internal Attribution</SectionTitle>
            <div className="mt-2 rounded-md bg-purple-50 p-2.5 space-y-2 text-[11px]">
              <div className="flex justify-between border-b border-purple-100 pb-1">
                <span className="text-purple-700/70">Vendor ID:</span>
                <span className="font-mono text-purple-900">{order.vendor_id || "None (Platform)"}</span>
              </div>
              <div className="flex justify-between border-b border-purple-100 pb-1">
                <span className="text-purple-700/70">Dropshipper ID:</span>
                <span className="font-mono text-purple-900">{order.dropshipper_id || "None"}</span>
              </div>
              <div className="flex justify-between border-b border-purple-100 pb-1">
                <span className="text-purple-700/70">Dropshipper Code:</span>
                <span className="font-mono text-purple-900 font-bold">{order.dropshipper_code || "None"}</span>
              </div>
              <div className="flex justify-between border-b border-purple-100 pb-1">
                <span className="text-purple-700/70">Affiliate Code:</span>
                <span className="font-mono text-purple-900">{order.affiliate_code || "None"}</span>
              </div>
              <div className="pt-1 text-[10px] italic text-purple-600">
                Verify this mapping against the "Dropshipper/Affiliate" card below.
              </div>
            </div>
          </Surface>
          ...
          <Surface>
            <SectionTitle icon={Clock}>Order Activity & Timeline</SectionTitle>
            <OrderTimeline orderId={order.id} />
            <div className="mt-6 space-y-1 border-t border-slate-50 pt-3 text-[10px] text-slate-400 uppercase tracking-widest">
