...
function ProductsAdmin() {
  ...
  async function save(nextItem?: Partial<DBProduct>) {
    ...
    const payload: any = {
      ...
      // Platform-owned products have no vendor. Only send a vendor_id when a real
      // vendor is linked, otherwise leave it NULL (the column is nullable).
    ...
    if (error) {
      ...
      if (msg.includes("products_vendor_id_fkey")) {
        return toast.error("This product is linked to a store that no longer exists. Pick an existing vendor or leave the vendor empty for a platform product.");
      }
      if (/row-level security|permission denied/i.test(msg)) {
        return toast.error("You do not have admin access to save products. Ask an administrator to grant you the admin role.");
      }
