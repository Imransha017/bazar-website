...
function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (!authUser) {
      ...
      window.location.href = "/auth?redirect=/checkout&mode=login";
    }
  ...
  const eta = isDhaka ? `${fmtDate(etaStart)} – ${fmtDate(etaEnd)} (২৪ ঘণ্টার মধ্যে)` : `${fmtDate(etaStart)} – ${fmtDate(etaEnd)}`;
    ...
    <SiteLayout>
      <div className="min-h-screen bg-muted/40">
        <div className="mx-auto max-w-6xl px-3 py-4 md:px-4 md:py-6">
          <h1 className="mb-4 text-lg font-semibold md:text-xl">চেকআউট (Checkout)</h1>
            ...
            <div className="space-y-3">
              ...
              <section className="rounded bg-card shadow-card">
                <header className="flex items-center gap-2 border-b px-4 py-3 font-semibold">
                  <MapPin className="size-4 text-primary" /> শিপিং ঠিকানা (Shipping Address)
                </header>
                <div className="space-y-3 px-4 py-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div>
                      <label className="mb-1 block text-xs text-muted-foreground">আপনার নাম (Full Name) *</label>
                    ...
                    <div>
                      <label className="mb-1 block text-xs text-muted-foreground">ফোন নম্বর (Phone Number) *</label>
                    ...
                    <div>
                      <label className="mb-1 block text-xs text-muted-foreground">জেলা (District) *</label>
                      <SearchableSelect
                        id="field-region"
                        value={addr.region}
                    ...
                    <div>
                      <label className="mb-1 block text-xs text-muted-foreground">থানা / উপজেলা (Thana / Upazila) *</label>
                      <SearchableSelect
                        id="field-city"
                        value={addr.city}
                    ...
                    <div className="sm:col-span-2">
                      <label className="mb-1 block text-xs text-muted-foreground">বিস্তারিত ঠিকানা (Delivery Full Address) *</label>
                  ...
                  {items.map(({ product: p, qty }) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-start gap-3 px-4 py-3">
                        ...
                        <span className="text-[13px] md:text-xs font-semibold text-[#A52A2A] dark:text-[#E9967A] whitespace-nowrap">{formatBDT(p.price * qty)}</span>
                      </div>
                    );
                  })}
                </div>
                    ...
                    return (
                      <>
                        ...
                        <div className="mt-2 flex items-center justify-between border-t pt-2">
                          ...
                          <span className="text-lg font-bold text-[#A52A2A] dark:text-[#E9967A]">{formatBDT(total)}</span>
                        </div>
                        {totalSavings > 0 && (
