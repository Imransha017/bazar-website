...
type DropshipperAnalytics = { id: string; store_name: string; total_sales: number; order_count: number };
type VendorAnalytics = { id: string; store_name: string; total_sales: number; order_count: number };
...
function Analytics() {
  ...
  const [dsAnalytics, setDsAnalytics] = useState<DropshipperAnalytics[]>([]);
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const [o, r, c, ds, vn] = await Promise.all([
        supabase.from("orders").select("id,total,status,created_at,coupon_code,discount,customer_name,dropshipper_id,vendor_id").order("created_at", { ascending: false }),
        ...
        supabase.from("dropshippers").select("id,store_name"),
      ...
      ds.data?.forEach(d => dsMap.set(d.id, d.store_name));
      const vnMap = new Map<string, string>();
      vn.data?.forEach(v => vnMap.set(v.id, v.store_name));

      const dsStats = new Map<string, DropshipperAnalytics>();
      const vnStats = new Map<string, VendorAnalytics>();

      (o.data ?? []).forEach((ord: any) => {
        if (ord.status !== "cancelled") {
          if (ord.dropshipper_id) {
            const s = dsStats.get(ord.dropshipper_id) || { id: ord.dropshipper_id, store_name: dsMap.get(ord.dropshipper_id) || "Unknown", total_sales: 0, order_count: 0 };
            s.total_sales += Number(ord.total);
            s.order_count += 1;
            dsStats.set(ord.dropshipper_id, s);
          }
          if (ord.vendor_id) {
            const s = vnStats.get(ord.vendor_id) || { id: ord.vendor_id, store_name: vnMap.get(ord.vendor_id) || "Unknown", total_sales: 0, order_count: 0 };
            s.total_sales += Number(ord.total);
            s.order_count += 1;
            vnStats.set(ord.vendor_id, s);
          }
        }
      });

      setDsAnalytics(Array.from(dsStats.values()).sort((a, b) => b.total_sales - a.total_sales).slice(0, 10));
      setVendorAnalytics(Array.from(vnStats.values()).sort((a, b) => b.total_sales - a.total_sales).slice(0, 10));
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center gap-1 bg-white border rounded-lg px-2 py-1">
              ...
              {(dateRange.start || dateRange.end) && (
                <button onClick={() => setDateRange({ start: "", end: "" })} className="text-rose-500 hover:text-rose-700 ml-1">
                  <X className="h-3 w-3" />
                </button>
              )}
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard title="Top Dropshippers (Sales)" hint="Top 10 by total revenue">
          {dsAnalytics.length === 0 ? <Empty msg="No dropshipper sales yet" /> : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={dsAnalytics} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                <XAxis type="number" fontSize={10} />
                <YAxis dataKey="store_name" type="category" width={100} fontSize={10} />
                <Tooltip />
                <Bar dataKey="total_sales" fill="#d946ef" name="Sales (৳)" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>

        <ChartCard title="Top Vendors (Volume)" hint="Top 10 by total revenue">
          {vendorAnalytics.length === 0 ? <Empty msg="No vendor sales yet" /> : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={vendorAnalytics} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                <XAxis type="number" fontSize={10} />
                <YAxis dataKey="store_name" type="category" width={100} fontSize={10} />
                <Tooltip />
        ...
        <ChartCard title="Quick Comparison: DS vs Vendor Performance" full>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={dsAnalytics.slice(0, 5).map((ds, i) => ({
                name: ds.store_name,
                ds_sales: ds.total_sales,
                vn_sales: vendorAnalytics[i]?.total_sales || 0,
                vn_name: vendorAnalytics[i]?.store_name || "N/A"
              }))}
            >
              <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
              ...
              <Legend />
              <Bar dataKey="ds_sales" fill="#d946ef" name="Top Dropshippers" />
              <Bar dataKey="vn_sales" fill="#10b981" name="Top Vendors" />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
