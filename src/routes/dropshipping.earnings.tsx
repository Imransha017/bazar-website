...
import { DollarSign, Download, Wallet, Info } from "lucide-react";
import { ProfitLedgerModal } from "@/components/ProfitLedgerModal";
...
function EarningsPage() {
  ...
  const [viewLedger, setViewLedger] = useState<DropshipperEarning | null>(null);
  ...
  const filtered = useMemo(() => items.filter(i => {
    ...
    if (q) {
      ...
      return (i.product_id || "").toLowerCase().includes(s) || 
  ...
  const exportCsv = () => {
    const rows = [
      ["Date", "Order ID", "Product", "Qty", "Base", "Retail", "Profit", "Status", "History Log"],
      ...filtered.map(r => [
    ...
    const a = document.createElement("a"); a.href = url; a.download = `earnings-ledger-${Date.now()}.csv`; a.click(); URL.revokeObjectURL(url);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-xl border bg-card p-3">
        ...
        <div className="flex-1 min-w-[180px]">
          <label className="block text-[10px] font-bold text-muted-foreground uppercase">Search Order / ID</label>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Order ID or Product ID…" className="w-full rounded border px-2 py-1.5 text-xs focus:ring-1 focus:ring-primary outline-none" />
        </div>
              ...
              <tr key={r.id} className="border-t">
                ...
                <td className="p-2 text-center">
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full p-1.5 text-slate-400 hover:bg-slate-100 hover:text-primary transition-colors"
                    title="View profit ledger history"
                  >
