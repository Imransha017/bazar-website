...
import {
  ...
  Truck, ExternalLink, User, StickyNote, Store, FileText, Clock,
} from "lucide-react";
...
function OrderPage() {
  ...
  const [dropshippers, setDropshippers] = useState<Record<string, { store_name: string; phone: string; code: string }>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const dsids = Array.from(new Set(rows.map(r => r.dropshipper_id).filter(Boolean))) as string[];
      if (dsids.length) {
        const { data: dss } = await supabase.from("dropshippers").select("id, store_name, phone, code, user_id").in("id", dsids);
        if (dss) {
          const m: Record<string, any> = {};
          dss.forEach(d => m[d.id] = { store_name: d.store_name, phone: d.phone, code: d.code });
          setDropshippers(m);
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    if (searchParams.get('download') === 'true' && (order || dbRows.length > 0)) {
      // Small delay to ensure data is loaded
      const timer = setTimeout(() => {
        downloadAll();
        // Clear param to prevent repeated downloads on re-render
  ...
  const buildInvoice = (row: DBOrderRow): InvoiceData => ({
    ...
    track_url: typeof window !== "undefined" ? window.location.href : null,
  ...
  return (
    <SiteLayout>
      <div className="mx-auto max-w-4xl px-3 py-6 md:px-4 space-y-4">
        ...
        {/* Invoice + cancel/revision requests per sub-order */}
        {dbRows.map((r) => (
          <div key={`req-${r.order_number}`} className="space-y-4">
            <section className="rounded-lg border-2 border-primary/20 bg-white p-4 shadow-card print:hidden">
              <div className="mb-3 flex items-center gap-2 border-b border-primary/10 pb-2 text-sm font-bold uppercase tracking-wider text-primary">
                <FileText className="size-4" /> ইনভয়েস — #{r.order_number}
              </div>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm">
                  <div>স্ট্যাটাস: <span className="font-bold capitalize">{r.status}</span></div>
                  <div className="text-muted-foreground">মোট: {formatBDT(Number(r.total))}</div>
                </div>
                <button
                  onClick={() => openPrintableInvoice(buildInvoice(r))}
                  className="flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground shadow hover:opacity-90"
                >
                  <Download className="size-4" /> ইনভয়েস PDF ডাউনলোড
                </button>
              </div>
              <p className="mt-2 text-[11px] text-muted-foreground">
                বাটনে ক্লিক করলে প্রিন্ট উইন্ডো খুলবে — সেখান থেকে "Save as PDF" নির্বাচন করে ডাউনলোড করুন।
              </p>
            </section>
            <OrderRequestForm orderNumber={r.order_number} phone={r.customer_phone} status={r.status} />
          </div>
        ))}
        ...
        {/* Dropshipper details */}
        {dbRows.some(r => r.dropshipper_id) && (
          <section className="rounded-lg border-2 border-fuchsia-200 bg-white p-4 shadow-card">
            <div className="mb-3 flex items-center gap-2 border-b border-fuchsia-100 pb-2 text-sm font-bold uppercase tracking-wider text-fuchsia-700">
              <Store className="size-4" /> Dropshipper Details
            </div>
            <div className="space-y-2 text-sm">
              {dbRows.map((r) => {
                const d = r.dropshipper_id ? dropshippers[r.dropshipper_id] : null;
                if (!d) return null;
                return (
                  <div key={r.order_number} className="rounded border border-fuchsia-100 bg-fuchsia-50/40 p-3">
                    <div className="font-bold text-fuchsia-900">{d.store_name}</div>
                    <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-fuchsia-800">
                      <div>Code: {d.code}</div>
        ...
        <section className="rounded-lg border-2 border-slate-200 bg-white p-4 shadow-card">
          <div className="mb-3 flex items-center gap-2 border-b border-slate-100 pb-2 text-sm font-bold uppercase tracking-wider text-slate-700">
            <Clock className="size-4" /> Order Status Tracking
          </div>
          <div className="space-y-4">
            {dbRows.map(r => (
              <div key={r.id} className="rounded-lg border border-slate-100 bg-slate-50/30 p-3">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-mono font-bold text-slate-500">Order #{r.order_number}</span>
