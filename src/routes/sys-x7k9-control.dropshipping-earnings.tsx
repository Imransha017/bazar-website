...
function AdminDsEarnings() {
  ...
  const exportCsv = () => {
    const csv = [
      ["Date", "Store", "Code", "Order", "Product", "Qty", "Base", "Retail", "Profit", "Status", "History Log"],
      ...filtered.map(r => [
        new Date(r.created_at).toISOString(),
        r.dropshippers?.store_name ?? "",
        r.dropshippers?.code ?? "",
        r.order_id,
        r.product_id ?? "",
        String(r.qty),
        String(r.base_price),
        String(r.retail_price),
        String(r.profit),
        r.status,
        (r.activity_log || []).map(l => `[${new Date(l.changed_at).toLocaleString()}] ${l.status.toUpperCase()}: ${l.note}`).join(" | ")
      ]),
    ].map(r => r.map(c => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    const a = document.createElement("a"); a.href = url; a.download = `admin-earnings-ledger-${Date.now()}.csv`; a.click(); URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <div className="flex flex-wrap items-center gap-1.5 border rounded-lg p-1 bg-white">
          {["all", "pending", "approved", "rejected", "paid"].map(f => (
            <button key={f} onClick={() => setStatus(f)} className={`rounded px-3 py-1 text-xs font-bold capitalize transition-colors ${status === f ? "bg-purple-700 text-white" : "hover:bg-slate-100"}`}>{f}</button>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase">From</label>
            <input type="date" value={from} onChange={e => setFrom(e.target.value)} className="rounded border px-2 py-1.5 text-xs focus:ring-1 focus:ring-purple-500 outline-none" />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase">To</label>
            <input type="date" value={to} onChange={e => setTo(e.target.value)} className="rounded border px-2 py-1.5 text-xs focus:ring-1 focus:ring-purple-500 outline-none" />
          </div>
        </div>
        <div className="flex-1 min-w-[240px]">
          <label className="block text-[10px] font-bold text-slate-400 uppercase">Search Store / Order / Transaction ID</label>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Store, Order ID, Product ID, or Transaction ID…" className="w-full rounded border px-2 py-1.5 text-xs focus:ring-1 focus:ring-purple-500 outline-none" />
        </div>
      </div>
                ...
                <tr ref={el => { rowRefs.current[r.id] = el; }} className={`border-t border-slate-100 hover:bg-purple-50/30 ${highlight === r.id ? "bg-amber-50 ring-2 ring-amber-400" : ""}`}>
                  ...
                  <td className="pr-3 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <button onClick={() => setOpenLog(openLog === r.id ? null : r.id)} className="inline-flex items-center gap-0.5 rounded border px-2 py-1 text-[10px] font-bold hover:bg-slate-50"><History className="h-3 w-3" />Details</button>
                    </div>
                  </td>
                </tr>
                {openLog === r.id && (
                  <tr key={r.id + "-log"} className="bg-slate-50/70 border-b">
                    <td colSpan={10} className="p-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <h4 className="text-[10px] font-bold uppercase text-slate-400 mb-2">Audit Log</h4>
                          <AuditLog entityType="dropshipper_earning" entityId={r.id} compact />
                        </div>
                        <div>
                          <h4 className="text-[10px] font-bold uppercase text-slate-400 mb-2">Profit Ledger History</h4>
                          <div className="space-y-3">
                            {(r.activity_log || []).length === 0 ? (
                              <p className="text-xs text-slate-400 italic">No ledger activity recorded.</p>
                            ) : (
                              (r.activity_log || []).map((entry, idx) => (
                                <div key={idx} className="flex gap-2 text-xs">
                                  <div className="flex flex-col items-center">
                                    <div className={`size-2 rounded-full mt-1 ${
                                      entry.status === 'approved' ? 'bg-green-500' :
                                      entry.status === 'rejected' ? 'bg-red-500' :
                                      'bg-amber-500'
                                    }`} />
                                    {idx < (r.activity_log || []).length - 1 && <div className="w-px flex-1 bg-slate-200 my-1" />}
                                  </div>
                                  <div>
                                    <div className="flex gap-2 items-center">
                                      <span className="font-bold uppercase text-[9px]">{entry.status}</span>
                                      <span className="text-[9px] text-slate-400">{new Date(entry.changed_at).toLocaleString()}</span>
                                    </div>
