    ...
    <div className="max-w-2xl space-y-4">
      ...
      <div className="space-y-3 rounded-lg bg-card p-5 shadow-sm">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Choose how you want to be alerted about low stock.</p>
        </div>
        <div className="space-y-2 pt-2">
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              <div className="text-sm font-bold">In-App Notifications</div>
              <div className="text-[10px] text-muted-foreground">Receive alerts in your dashboard bell icon.</div>
            </div>
            <input 
              type="checkbox" 
              checked={v.notification_preferences?.low_stock_app ?? true} 
              onChange={e => setV({ ...v, notification_preferences: { ...(v.notification_preferences ?? { low_stock_app: true, low_stock_email: false }), low_stock_app: e.target.checked } })}
          ...
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              ...
              <div className="text-[10px] text-muted-foreground">Get an email when stock is low.</div>
            </div>
            <input 
              type="checkbox" 
              checked={v.notification_preferences?.low_stock_email ?? false} 
              onChange={e => setV({ ...v, notification_preferences: { ...(v.notification_preferences ?? { low_stock_app: true, low_stock_email: false }), low_stock_email: e.target.checked } })}
              className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
            />
          </div>
        </div>
