  ...
  return (
    <div className="rounded-xl border border-purple-200 bg-gradient-to-br from-purple-50/70 to-indigo-50/50 p-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="text-xs font-bold text-purple-800">🗂️ Product Category Selection</div>
        <div className="text-[10px] text-slate-500 font-medium bg-white/50 px-2 py-1 rounded-md border border-purple-100 italic">
          Parent categories are assigned automatically based on your choice.
        </div>
      </div>

      {mode === "single" ? (
        <div className="grid gap-3 sm:grid-cols-2">
          {/* Category selection hidden as per requirement */}
          ...
          <Field label="Subcategory *">
            <select
              ...
              onChange={(e) => {
                ...
                const sub = cats.find(c => c.slug === subSlug);
                const parent = sub ? parents.find(p => p.id === sub.parent_id) : null;
                
                if (parent) {
                  // Automatically set parent category
                  const catSlugs = [parent.slug];
                  const catNames = [parent.name];
                  const subNames = sub ? [sub.name] : [];
                  
                  setItem({
                    ...item,
                    category_slug: joinSlugs(catSlugs),
                    category_name: catNames.join(", "),
                    subcategory_slug: joinSlugs([subSlug]),
                    subcategory_name: subNames.join(", "),
                    option_slug: "",
                    option_name: "",
                  } as Partial<DBProduct>);
                } else {
                  setSubcategories([subSlug]);
                }
              }}
              className="input"
            >
              <option value="">— Select subcategory —</option>
              {cats.filter(c => c.parent_id).map((s) => (
                <option key={s.id} value={s.slug}>
                  {parents.find(p => p.id === s.parent_id)?.name} → {s.name}
                </option>
              ))}
            </select>
          </Field>

          {/* Option selection hidden as per requirement (or implicit "category & subcategory" focus) */}
          <div className="hidden">
            <Field label="Option">
        ...
        <div className="grid gap-3 sm:grid-cols-1">
          {/* Multiple mode - Only show Subcategories list */}
          <div className="hidden">
            <div className="mb-1 text-[11px] font-semibold text-purple-800">Categories *</div>
            <div className="max-h-44 space-y-1 overflow-y-auto rounded-lg border border-purple-200 bg-white p-2">
              {parents.map((p) => {
                ...
                return (
                  <label key={p.id} className={`flex items-center gap-2 px-2 py-1 text-xs ${checked ? "bg-purple-100 font-semibold text-purple-900" : ""}`}>
                    <input type="checkbox" checked={checked} readOnly className="h-3.5 w-3.5 accent-purple-600" />
          ...
          <div>
            <div className="mb-1 text-[11px] font-semibold text-purple-800">Select Subcategories *</div>
            <div className="max-h-60 space-y-1 overflow-y-auto rounded-lg border border-purple-200 bg-white p-2">
              {parents.map((p) => {
                const list = cats.filter((s) => s.parent_id === p.id);
                if (list.length === 0) return null;
                return (
                  <div key={p.id} className="mb-2 border-b border-slate-50 pb-2 last:border-0">
                    <div className="px-1 pb-1 text-[10px] font-bold uppercase tracking-wide text-purple-400">{p.name}</div>
                    <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                      {list.map((s) => {
                        const checked = selectedSubSlugs.includes(s.slug);
                        return (
                          <label key={s.id} className={`flex cursor-pointer items-center gap-2 rounded-md px-2 py-1 text-xs transition-colors ${checked ? "bg-indigo-100 font-semibold text-indigo-900" : "hover:bg-slate-50"}`}>
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => {
                                const newSubSlugs = toggle(selectedSubSlugs, s.slug);
                                const activeSubs = cats.filter(c => newSubSlugs.includes(c.slug));
                                const parentIds = new Set(activeSubs.map(sub => sub.parent_id));
                                const activeParents = parents.filter(parent => parentIds.has(parent.id));
                                
                                setItem({
                                  ...item,
                                  category_slug: joinSlugs(activeParents.map(p => p.slug)),
                                  category_name: activeParents.map(p => p.name).join(", "),
                                  subcategory_slug: joinSlugs(newSubSlugs),
                                  subcategory_name: activeSubs.map(sub => sub.name).join(", "),
                                  option_slug: "",
                                  option_name: "",
                                } as Partial<DBProduct>);
                              }}
          ...
          <div className="hidden">
            <div className="mb-1 text-[11px] font-semibold text-purple-800">Options</div>
            {/* Options hidden as per instructions */}
          </div>
        </div>
      )}
    </div>
