    ...
    (async () => {
      ...
      const run = async (withVendor: boolean) => {
        let q = supabase
          ...
          .neq("id", currentId)
          .limit(20);
        // category_slug may hold a comma-separated multi-select list
        const primaryCat = String(categorySlug || "").split(",")[0].trim();
        if (primaryCat) q = q.ilike("category_slug", `%${primaryCat}%`);
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-1.5">
                ...
                <div className="mt-1 flex items-baseline gap-1">
                  <span className="text-[13px] md:text-[12px] font-bold text-[#A52A2A] dark:text-[#E9967A] leading-none whitespace-nowrap">৳{price.toLocaleString("en-BD")}</span>
