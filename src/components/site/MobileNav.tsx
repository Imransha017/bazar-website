    ...
    <nav className="fixed bottom-0 left-0 right-0 z-30 grid grid-cols-5 border-t bg-card shadow-[0_-2px_8px_rgba(0,0,0,0.08)] md:hidden">
      ...
      <Link to="/cart" className={itemCls} activeProps={activeCls}>
        <div className="relative">
          ...
          {count > 0 && (
            <span className="absolute -right-2 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-brand-dark px-1 text-[9px] font-bold leading-none text-brand-foreground shadow-sm tabular-nums">
              {count > 99 ? "99+" : count}
            </span>
          )}
        </div>
