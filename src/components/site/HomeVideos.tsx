...
export function HomeVideos() {
  ...
  const config = settings.homepage.videos_config;
  ...
  return (
    <section className="mx-auto max-w-none pt-3">
      <div className="overflow-hidden rounded-lg bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-3 shadow-card md:p-4">
        ...
        <div className={config?.layout === 'carousel' ? "flex gap-2 overflow-x-auto no-scrollbar" : "grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-3"}>
          {items.map((v) => {
            const yt = youtubeEmbed(v.image_url);
            const isVideo = /\.(mp4|webm|ogg)(\?|$)/i.test(v.image_url) || !!yt;
            return (
              <div key={v.id} className={`group relative overflow-hidden rounded-md bg-black shadow-lg ring-1 ring-white/10 ${config?.layout === 'carousel' ? 'w-[80vw] md:w-[45vw] shrink-0' : ''}`}>
                <div className={`relative ${config?.aspect_ratio === '9:16' ? 'aspect-[9/16]' : config?.aspect_ratio === '1:1' ? 'aspect-square' : 'aspect-video'}`}>
                  {yt ? (
                    <iframe
                      src={yt}
