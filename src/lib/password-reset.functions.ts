...
const SECRET_SEED = process.env["SUPABASE_SERVICE_ROLE_KEY"] || process.env["SESSION_SECRET"] || "bazar-secure-encryption-seed-key-32b";
const ENCRYPTION_KEY = crypto.scryptSync(SECRET_SEED, "bazar_salt_2026", 32);
...
/** Public: user submits a phone/email + desired new password. Stored securely with AES encryption. */
export const requestPasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Store encrypted password (AES-256-GCM) instead of plaintext/base64
    const encryptedSecret = encryptPassword(data.newPassword);

    const { error: insErr } = await supabaseAdmin.from("password_reset_requests").insert({
      identifier,
      method: data.method,
      user_id: foundId,
      new_password_hash: encryptedSecret,
      status: "pending",
    });
...
export const reviewPasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Approve → decrypt password and update via auth admin API.
    if (!row.user_id) throw new Error("User missing");
    const newPass = decryptPassword(row.new_password_hash);
    ...
    const { error } = await supabaseAdmin.from("password_reset_requests").update({
      ...
      reviewed_by: context.userId,
      reviewed_at: new Date().toISOString(),
      new_password_hash: "", // wipe out the secret
    }).eq("id", data.id);
    if (error) throw new Error(error.message);
