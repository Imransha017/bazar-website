...
/** Builds a QR code image (data URL) for the given text. Returns null on failure. */
...
export type InvoiceData = {
  ...
  site_address?: string;
  vendor_cost?: number;
  dropshipper_profit?: number;
  platform_fee?: number;
  is_admin_view?: boolean;
  /** Public link the QR code should point to (order tracking page). */
...
export function openPrintableInvoice(o: InvoiceData) {
  ...
  const trackUrl = o.track_url || o.tracking_url || `${origin}/order/${o.order_number}`;
  const qr = qrDataUrl(trackUrl);
  const qrBlock = qr
    ? `<div class="qrbox">
        <img class="qr" src="${qr}" alt="Order tracking QR code" />
        <div class="qrcap">স্ক্যান করে অর্ডার ট্র্যাক করুন</div>
        <div class="qrcap mono small">${esc(trackUrl)}</div>
      </div>`
    : "";

  const trackingBlock =
    o.courier_name || o.tracking_number || qr
      ? `<div class="box trackbox">
          <div style="flex:1">
            <div class="btitle">Shipping & Tracking</div>
            ${o.courier_name ? `<div><b>Courier:</b> ${esc(o.courier_name)}</div>` : ""}
...
<style>
  * { box-sizing: border-box; }
  ...
  .trackbox { display: flex; gap: 14px; align-items: center; justify-content: space-between; margin-bottom: 16px; }
  .qrbox { text-align: center; flex: 0 0 auto; }
  .qr { width: 108px; height: 108px; display: block; margin: 0 auto; }
  .qrcap { font-size: 9px; color: #64748b; margin-top: 3px; max-width: 150px; word-break: break-all; }
...
<div class="wrap">
  ...
  ${
    o.is_admin_view && (o.vendor_cost || o.dropshipper_profit)
      ? `
  <div class="box" style="margin-top: 20px; background: #f8fafc; border-style: dashed;">
    <div class="btitle">Settlement Details (Internal)</div>
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px;">
      <div>
        <div style="font-size: 10px; color: #64748b;">Vendor Cost</div>
        <div style="font-weight: 700;">${money(o.vendor_cost)}</div>
      </div>
      <div>
        <div style="font-size: 10px; color: #64748b;">DS Profit</div>
        <div style="font-weight: 700;">${money(o.dropshipper_profit)}</div>
      </div>
      <div>
        <div style="font-size: 10px; color: #64748b;">Platform Fee</div>
        <div style="font-weight: 700; color: #16a34a;">${money(o.platform_fee)}</div>
      </div>
    </div>
  </div>`
      : ""
