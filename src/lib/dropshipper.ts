...
export type Dropshipper = {
  ...
  real_time_popups_enabled?: boolean;
  theme_color_primary?: string;
  theme_color_background?: string;
  theme_layout_style?: string;
...
export type DropshipperEarning = {
  ...
  activity_log: Array<{ status: string; changed_at: string; note: string; previous_status?: string }> | null;
  created_at: string;
...
export function buildDsShortLink(slug: string, opts?: { origin?: string; alias?: string }) {
  const origin = opts?.origin ?? (typeof window !== "undefined" ? window.location.origin : "");
  if (opts?.alias) return `${origin}/go/${opts.alias}`;
  return `${origin}/ds/${slug}`;
}


export const dsSlugify = (s: string) =>
  s.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 20) || `s${Date.now().toString(36)}`;

export const dsCodeify = (s: string) =>
  s.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 10) || `d${Date.now().toString(36)}`;
...
export async function applyAsDropshipper(input: {
  ...
  const slug = `${base}-${Math.random().toString(36).slice(2, 4)}`;
  const code = dsCodeify(input.store_name) + Math.random().toString(36).slice(2, 4);
...
export async function updateMyDropshipper(patch: Partial<Pick<Dropshipper, "store_name" | "bio" | "phone" | "whatsapp" | "payout_method" | "payout_number" | "logo_url" | "banner_url" | "profile_image_url" | "whatsapp_order_enabled" | "real_time_popups_enabled" | "theme_color_primary" | "theme_color_background" | "theme_layout_style">>) {
  const { data: { user } } = await supabase.auth.getUser();
...
export async function trackDsClick(code: string, productId?: string | null) {
  try {
    ...
    const utmSource = searchParams.get("s") || searchParams.get("utm_source");
    const utmCampaign = searchParams.get("c") || searchParams.get("utm_campaign");
    const utmMedium = searchParams.get("m") || searchParams.get("utm_medium");
...
export async function getPublicStore(slug: string) {
  const { data: ds } = await db
    ...
    .select("id,code,store_name,store_slug,logo_url,banner_url,profile_image_url,bio,status,whatsapp,whatsapp_order_enabled,real_time_popups_enabled,theme_color_primary,theme_color_background,theme_layout_style,visibility_mode")
    .eq("store_slug", slug)
    .maybeSingle();
  ...
  const { data: products } = await db.from("products")
    .select("id,slug,name,price,original_price,image,gallery,description,category_slug,category_name,subcategory_slug,subcategory_name,rating,sold_count")
    .in("id", ids)
    .eq("is_active", true);
    
  const pMap = new Map((products ?? []).map((p: any) => [p.id, {
    ...p,
