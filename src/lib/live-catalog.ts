...
import { type Product, type Category, type Subcategory, categories as fallbackCategories } from "@/lib/data";
import { normalizeProductImage } from "@/lib/admin-api";
import { slugList, nameList, pairSlugNames, hasSlug } from "@/lib/slugs";
...
export function toLiveProduct(row: any): Product {
  ...
  const catSlugs = slugList(row.category_slug);
  const catNames = nameList(row.category_name);
  const subSlugs = slugList(row.subcategory_slug);
  const subNames = nameList(row.subcategory_name);
  const optSlugs = slugList(row.option_slug);
  return {
    id: row.id,
    ...
    category: catSlugs[0] || "uncategorized",
    categoryName: decodeHtml(catNames[0] || "") || undefined,
    subcategory: subSlugs[0] || undefined,
    subcategoryName: decodeHtml(subNames[0] || "") || undefined,
    categories: catSlugs,
...
export function deriveLiveCategories(rows: any[]): Category[] {
  ...
  for (const r of rows) {
    // A product may be tagged with multiple categories / subcategories.
    const cats = pairSlugNames(r.category_slug, r.category_name);
    const subs = pairSlugNames(r.subcategory_slug, r.subcategory_name);
    const list = cats.length ? cats : [{ slug: "uncategorized", name: "Uncategorized" }];
    for (const c of list) {
      const name = decodeHtml(c.name || humanize(c.slug));
      if (!map.has(c.slug)) map.set(c.slug, { name, subs: new Map() });
      for (const s of subs) {
        map.get(c.slug)!.subs.set(s.slug, decodeHtml(s.name || humanize(s.slug)));
      }
    }
  }
...
async function fetchLiveCatalog(): Promise<{ products: Product[]; categories: Category[] }> {
  try {
    const [prodRes, catRes, revRes] = await Promise.all([
      supabase
        .from("products")
        .select("id,slug,name,sku,tags,price,original_price,dropshipper_price,rating,sold_count,category_slug,category_name,subcategory_slug,subcategory_name,option_slug,brand,image,gallery,description,is_active")
        .eq("is_active", true)
    ...
    if (prodRes.error) {
      console.error("[live-catalog] Error fetching products:", prodRes.error.message);
      throw prodRes.error;
    }
    ...
    console.error("[live-catalog] Critical failure in fetchLiveCatalog:", err);
