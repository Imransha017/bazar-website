...
export async function updateMyVendor(patch: Partial<Pick<Vendor, "store_name" | "description" | "phone" | "address" | "logo_url" | "banner_url" | "footer" | "notification_preferences">>) {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error("Sign in required");
  const { error } = await supabase.from("vendors").update(patch as any).eq("user_id", user.id);
  if (error) throw error;
}
