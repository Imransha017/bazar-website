...
import { notifyOrderPlaced } from "./notify.functions";
...
export function OrdersProvider({ children }: { children: ReactNode }) {
  ...
  const placeOrder: Ctx["placeOrder"] = (o) => {
    ...
    import("@/lib/admin-api").then(async ({ createDBOrder, getProductVendorMap }) => {
      ...
      let dsCodeToUse = dsCode;
      
      if (dsCodeToUse) {
        try {
          const { supabase } = await import("@/integrations/supabase/client");
          const { data } = await supabase.from("dropshippers").select("id").eq("code", dsCodeToUse).maybeSingle();
          if (data) dsId = data.id;
        } catch { /* ignore */ }
      }

      // Fetch base prices for dropshipper attribution
      ...
      for (const [vendor_id, items] of groups) {
        ...
        try {
          const row = await createDBOrder({
            ...
            vendor_id,
            dropshipper_id: dsId,
            dropshipper_code: dsCode,
          });
          if (row?.order_number) dbNumbers.push(row.order_number);
          if (row?.id) createdOrderIds.push(row.id);
          if (refCode && row?.id) {
      ...
      // Notify customer, vendor and dropshipper (in-app + email)
      if (createdOrderIds.length) {
        try {
          for (const oid of createdOrderIds) {
            // We await it to ensure it finishes or at least starts reliably, 
            // but we don't let it crash the main order success flow
            await notifyOrderPlaced({ data: { orderId: oid } }).catch(console.error);
          }
        } catch (e) {
          console.error("Failed to trigger order notifications:", e);
        }
      }


      if (createdOrderIds.length && typeof window !== "undefined") {
        const alias = new URLSearchParams(window.location.search).get('a');
        if (alias) {
          import("./marketing-pro.functions").then(({ trackShortLinkEvent }) => {
            trackShortLinkEvent({ data: { alias, type: 'order' } }).catch(console.error);
          });
        }
      }
